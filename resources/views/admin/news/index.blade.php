    ...
    <div class="col-lg-9 pt-4 pb-2 pb-sm-4">
      <div class="d-flex align-items-center mb-4">
        <h1 class="h2 mb-0">Blogs</h1>
          <div class="ms-auto">
              <a class="btn btn-sm btn-primary ms-auto" style="float: right;" href="{{route('show_create_news')}}">Post Blog</a>
          </div>
      </div>
      ...
      <div class="card border-0 py-1 p-md-2 p-xl-3 p-xxl-4">
        <div class="card-body pb-4">
            ...
            @if (count($news) > 0)
                @foreach ($news as $n)
                    <tr>
                        <td class="text-dark">{{$n->title}}</td>
                        <td>{{$n->description}}</td>
                        <td>{{date('M d, Y', strtotime($n->created_at))}}</td>
                        <td>
                            <a  onclick="event.preventDefault(); window.location.href='/news/{{$n->id}}/news-info';" class="nav-link text-default fs-sm fw-normal text-primary py-1 pe-0 ps-1 ms-2" type="button" data-bs-toggle="tooltip" aria-label="View">
                                View
                            </a>
                            ...
                            <form id="b-form{{$n->id}}" action="{{route('delete_news', ['news'=>$n->id])}}" method="POST" class="d-none">
                                @csrf
                            </form>
                        </td>
                    </tr>
