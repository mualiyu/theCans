...
@section('content')
    ...
    <div class="col-lg-9 pt-4 pb-2 pb-sm-4">
      <div class="d-flex align-items-center mb-4">
        <h1 class="h2 mb-0">FAQ's</h1>
          <div class="ms-auto">
              <a class="btn btn-sm btn-primary ms-auto" style="float: right;" href="{{route('show_create_faq')}}">Add FAQ</a>
          </div>
      </div>
            ...
            @if (count($faqs) > 0)
                @foreach ($faqs as $n)
                    <tr>
                        ...
                        <td>
                            <a  onclick="event.preventDefault(); window.location.href='/faqs/{{$n->id}}/faq-info';" class="nav-link text-default fs-sm fw-normal text-primary py-1 pe-0 ps-1 ms-2" type="button" data-bs-toggle="tooltip" aria-label="View">
                                View
                            </a>
                            ...
                            <form id="b-form{{$n->id}}" action="{{route('delete_faq', ['faq'=>$n->id])}}" method="POST" class="d-none">
                                @csrf
                            </form>
                        </td>
                    </tr>
